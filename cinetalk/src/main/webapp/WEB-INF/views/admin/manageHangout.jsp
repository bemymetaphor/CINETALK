...
<div class="container-fluid">
	...
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
		<div>
			<h1 style="color:#CD5C5C">모임 게시판 관리</h1>
			<h6 class="text-muted">${hCount }개의 글이 있습니다.</h6>
		</div>
	</div>
	<form action="${contextPath }/searchHangout.ad">
	...
	<table class="table">
		...
		<tbody>
			<c:forEach var="h" items="${ hList }">
				<tr>
					<td><input name = "selectDelete" type = "checkbox" value = "${h.boardId }"/></td>
					<td>${h.boardId }</td>
					<td>${h.boardWriter }</td>
					<td><a href="<c:url value='/hangoutDetail.ha?boardId=${h.boardId}'/>">${h.boardTitle }</a></td>
					<td>${h.modifyDate }</td>
				</tr>
			</c:forEach>
		</tbody>
	...
	function deleteValue() {
		...
		if(deleteArr.length == 0) {
			...
			$.ajax({
				url: "${contextPath}/deleteHangout.ad",
				type: 'POST',
				data: {
					deleteArr: deleteArr
				},
