...
<div class="container-fluid">
	...
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
		<div>
			<h1 style="color:#CD5C5C">신고 관리</h1>
			
		</div>
	</div>
	<h3>리뷰 신고</h3>
	<h6 class="text-muted">${ rCount }개의 신고가 있습니다.</h6>
	...
	<table class="table">
		<thead>
			<tr>
				...
				<th scope="col">상세 사유</th>
			</tr>
		</thead>
		<tbody>
			<c:forEach var="r" items="${ rrList }" varStatus="i">
				<tr>
					<td><input name = "selectDelete" type = "checkbox" value = "${rList[i.index].boardId }"/></td>
					<td>${rList[i.index].boardId }</td>
 					<td>${rList[i.index].boardWriter }</td>
 					<td><a href="<c:url value='/movieReviewDetail.re?boardId=${rList[i.index].boardId}&writer=${rList[i.index].nickName }&movieTitle=${mtList[i.index]}'/>">${rList[i.index].boardTitle }</a></td>
	...
	<h3>모임 신고</h3>
	<h6 class="text-muted">${ hCount }개의 신고가 있습니다.</h6>
	<table class="table">
		<thead>
			<tr>
				<th scope="col"></th>
		...
		<tbody>
			<c:forEach items="${hrList }" var = "hr" varStatus="i">
				<tr>
					<td><input name = "selectDelete" type = "checkbox" value = "${hList[i.index].boardId }"/></td>
					<td>${hList[i.index].boardId }</td>
					<td>${hList[i.index].boardWriter }</td>
 					<td><a href="<c:url value='/hangoutDetail.ha?boardId=${hList[i.index].boardId }&writer=${hList[i.index].boardWriter }'/>">${hList[i.index].boardTitle }</a></td>
					<td>${hr.reportCategory }</td>
					<td>${hr.reportContent }</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	...
	function deleteValue() {
		var deleteArr = new Array(); //삭제할 신고 리스트
		...
		if(deleteArr.length == 0) {
			...
			$.ajax({
				url: "${contextPath}/deleteReportBoard.ad",
				type: 'POST',
				data: {
					deleteArr: deleteArr
				},
